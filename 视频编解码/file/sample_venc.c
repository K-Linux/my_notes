...
HI_S32 SAMPLE_VENC_1080P_CLASSIC(HI_VOID)
{
    ...
    ******************************************/
    ...
    {
        // A分辨率的内存池/视频buffer
        // 计算一个块（一帧）的长度(包含头)[默认YUV420SP格式]
	    u32BlkSize = SAMPLE_COMM_SYS_CalcPicVbBlkSize(gs_enNorm,\
	                enSize[0], SAMPLE_PIXEL_FORMAT, SAMPLE_SYS_ALIGN_WIDTH);
	    stVbConf.astCommPool[0].u32BlkSize = u32BlkSize;    //A内存池中每个块的长度(等于每帧的长度)
	    stVbConf.astCommPool[0].u32BlkCnt = g_u32BlkCnt;    //A内存池中共4个块
	}
	if(s32ChnNum >= 2)
    {
        // B分辨率的内存池/视频buffer
    ...
    ******************************************/
    ...
    /******************************************
     step 3: start vi dev & chn to capture          //vi 由 dev 和 channel 组成
    ******************************************/
    ...
    ******************************************/
    /*
     * VPSS（Video Process Sub-System）支持对一幅输入图像进行统一预处理，如去噪、去隔行、
     * 翻转、等，然后再对各通道分别进行缩放、锐化等处理，最后输出多种不同分辨率的图像。
     */

    // 将 dev0_chn0 和 group0_chn0 绑定，dev0_chn0 作为输入源，输出不同的分辨率 group0_chn0、group0_chn1 和 group0_chn2
...
int main(int argc, char *argv[])
{
    ...
    // printf("\033[41;30mdebuglog:\033[0m %s:%d [START]\n", __FILE__, __LINE__);
