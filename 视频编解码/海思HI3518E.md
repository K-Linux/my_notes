...
编译ORTP库前，需要在`avprofile.c`里的`av_profile_init()`添加一行`rtp_profile_set_payload(profile,96,&payload_type_h264);`
...
- ./autogen.sh
- ./configure --prefix=/home/k/samba/ortp-master/install --host=arm-hisiv300-linux
- make && make install

#### <font color="1E90FF" size="3">ORTP开发板运行</font>

- 将编译后ORTP的头文件目录拷贝到 `mpp/inlcude/` 路径下（注意拷贝的是一个目录，而非多个文件）
- 将编译后ORTP的lib目录拷贝到 `mpp/lib/rtp_lib/` 路径下
- 修改`mpp/sample/venc/Makefile`，添加库名`-lortp`和库路径`-L../../lib/ortp_lib`
- 编译出`sample_venc`应用程序
- 将`libortp.so` `libortp.so.9` `libortp.so.9.0.0`拷贝到开发板的`/usr/lib`目录中
- 开发板中运行`sample_venc`应用程序
